...
class App extends Component {
	...
	createNote() { this.setState({create: true}) }
	updateNote(id) { this.setState({updateID: id}) }
	searchNote(query) { this.setState({query}) }
	...
	render() {
		const filteredNotes = this.state.notes.filter((note) =>
				(note.title.toLowerCase().indexOf(this.state.query.toLowerCase()) !== -1)||
